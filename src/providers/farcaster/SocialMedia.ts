...
export class FarcasterSocialMedia implements Provider {
    ...
    async createSession(): Promise<FarcasterSession> {
        ...
        const { result: user } = await fetchJSON<UserResponse>(
            urlcat(process.env.WARPCAST_ROOT_URL, '/me', {
                method: 'GET',
                headers: {
    ...
    async discoverPosts(indicator?: PageIndicator) {
        ...
        const url = urlcat(process.env.WARPCAST_ROOT_URL, '/casts', {
    ...
    async getPostById(postId: string) {
        ...
        const url = urlcat(process.env.WARPCAST_ROOT_URL, '/cast', { hash: postId });
    ...
    async getProfileById(profileId: string) {
        ...
        const url = urlcat(process.env.WARPCAST_ROOT_URL, '/user', { fid: profileId });
    ...
    async getPostsByParentPostId(parentPostId: string, indicator?: PageIndicator) {
        ...
        const url = urlcat(process.env.WARPCAST_ROOT_URL, '/all-casts-in-thread', {
    ...
    async getFollowers(profileId: string, indicator?: PageIndicator) {
        ...
        const url = urlcat(process.env.WARPCAST_ROOT_URL, '/followers', {
            fid: profileId,
            limit: 10,
            cursor: indicator?.cursor,
        });
    ...
    async getFollowings(profileId: string, indicator?: PageIndicator) {
        ...
        const url = urlcat(process.env.WARPCAST_ROOT_URL, '/following', {
            fid: profileId,
            limit: 10,
            cursor: indicator?.cursor,
        });
    ...
    async upvotePost(postId: string) {
        ...
        const url = urlcat(process.env.WARPCAST_ROOT_URL, '/cast-likes');
    ...
    async commentPost(postId: string, comment: string) {
        ...
        const url = urlcat(process.env.WARPCAST_ROOT_URL, '/casts', { parent: postId });
    ...
    async mirrorPost(postId: string) {
        ...
        const url = urlcat(process.env.WARPCAST_ROOT_URL, '/recasts');
    ...
    async followProfile(profileId: string) {
        ...
        const url = urlcat(process.env.WARPCAST_ROOT_URL, '/follows');
